...
void main() {
  ...
  test(
    'prepareProject generates aggregate audio timeline from per-clip subtitles',
    () async {
      ...
      try {
        ...
        final aggregate =
            await DatabaseService.getPreferredAggregateAudioSubtitleFile(
              project.id,
            );
        final clips = await DatabaseService.getGlobalSubtitleClips(
          aggregate!.id,
        );

        expect(aggregate.sourceType, SubtitleSourceType.generatedAggregate);
        expect(clips, hasLength(4));
        expect(clips.map((clip) => clip.startMs).toList(), [
          1000,
          3000,
          5500,
          7000,
