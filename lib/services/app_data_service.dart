...
/// 统一管理应用的 data 持久化目录。
///
/// - Windows / 测试覆写：优先使用可执行程序同级 `data/`
/// - macOS：使用 `Application Support/.../data/`，避免向 `.app` 内写入
class AppDataService {
  AppDataService._();

  static const String _macBundleId = 'com.example.asrTools';
  ...
  static Future<Directory> dataRootDirectory() async {
    final base = await _persistentDataBaseDirectory();
  ...
  static Future<void> _preparePersistentDataLayoutInternal() async {
    ...
    await tempDirectory();
    await _migrateLegacySandboxMacDataIfNeeded();
    await _migrateLegacySettingsIfNeeded();
    await _migrateLegacyDatabaseIfNeeded();
  }

  static Future<void> _migrateLegacySandboxMacDataIfNeeded() async {
    ...
    final currentBase = await _persistentDataBaseDirectory();
    ...
    if (!await legacyBase.exists()) return;
    if (p.equals(currentBase.path, legacyBase.path)) return;

    final legacyDataRoot = Directory(p.join(legacyBase.path, dataDirName));
    if (!await legacyDataRoot.exists()) return;

    final targetDataRoot = await dataRootDirectory();
    await _copyDirectoryContentsIfMissing(legacyDataRoot, targetDataRoot);
  }
  ...
  static Future<Directory> _persistentDataBaseDirectory() async {
    if (_executableDirOverride != null &&
        _executableDirOverride!.trim().isNotEmpty) {
      return Directory(_executableDirOverride!);
    }
  ...
  static Future<void> _copyDirectoryContentsIfMissing(
    Directory source,
    Directory target,
  ) async {
    ...
    await for (final entity in source.list(
      ...
      if (entity is Directory) {
        ...
        final isEmpty =
            ...
            targetDir.listSync(followLinks: false).isEmpty;
        if (isEmpty) {
          await _copyDirectoryRecursively(entity, targetDir);
        }
