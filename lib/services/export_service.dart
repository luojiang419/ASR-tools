...
class ExportService {
          ...
          nest: () {
            ...
            for (final timeline in timelineList) {
              ...
              for (final track in timeline.externalAudioTracks) {
                if (!_hasExternalAudioTrack(track) ||
                    addedAssets.contains(track.mediaFileId)) {
                  continue;
                }
                builder.element(
                  'asset',
                  nest: () {
                    builder.attribute('id', 'a_${track.mediaFileId}');
                    builder.attribute('name', track.fileName);
                    builder.attribute(
                      'src',
                      _toFileUri(track.effectiveAudioPath),
                    );
                ...
                addedAssets.add(track.mediaFileId);
              }
            }
          },
        );
                  ...
                  nest: () {
                    ...
                    builder.element(
                      ...
                      nest: () {
                        ...
                        final maxExternalTracks = _maxExternalTrackCount(
                          timelineList,
                        );
                        for (
                          var trackPosition = 0;
                          trackPosition < maxExternalTracks;
                          trackPosition++
                        ) {
                          for (final timeline in timelineList) {
                            if (trackPosition >=
                                timeline.externalAudioTracks.length) {
                              continue;
                            }
                            final track =
                                timeline.externalAudioTracks[trackPosition];
                            if (!_hasExternalAudioTrack(track)) {
                              continue;
                            }
                            final audioStartFrame =
                                videoPositions[timeline.videoFileId]! +
                                _msToFrames(track.trimStartMs);
                            ...
                            builder.element(
                              ...
                              nest: () {
                                ...
                                builder.attribute(
                                  'start',
                                  _framesToTimecode(
                                    _msToFrames(track.effectiveSourceInMs),
                                  ),
                                );
                                builder.attribute(
                                  'lane',
  ...
  static Future<void> exportXmeml(
    ...
    final externalAudioClipIds =
        <String, String>{}; // videoFileId|mediaFileId -> audio clip id
    final videoFileIds = <String, String>{}; // videoFileId -> file id
    final externalAudioFileIds =
        <String, String>{}; // videoFileId|mediaFileId -> audio file id
    ...
    for (final t in timelineList) {
      ...
      for (final track in t.externalAudioTracks) {
        if (!_hasExternalAudioTrack(track)) {
          continue;
        }
        final trackKey = _externalTrackKey(t.videoFileId, track.mediaFileId);
        externalAudioClipIds[trackKey] = '$base $idSeq';
        idSeq++;
        final aBase = _safeId(track.fileName);
        externalAudioFileIds[trackKey] = '$aBase $idSeq';
        idSeq++;
      }
    }
    ...
    for (final t in timelineList) {
      ...
      final externalAudioClipIdsForTimeline = t.externalAudioTracks
          .where(_hasExternalAudioTrack)
          .map(
            (track) =>
                externalAudioClipIds[_externalTrackKey(
                  t.videoFileId,
                  track.mediaFileId,
                )],
          )
      ...
      for (final externalAudioClipId in externalAudioClipIdsForTimeline) {
        buf.writeln('            <link>');
        buf.writeln(
    ...
    final hasExternalTrack = timelineList.any(
      (timeline) => timeline.externalAudioTracks.any(_hasExternalAudioTrack),
    );
    if (hasExternalTrack) {
      final maxExternalTracks = _maxExternalTrackCount(timelineList);
      for (
        var trackPosition = 0;
        trackPosition < maxExternalTracks;
        trackPosition++
        ...
        buf.writeln('        <track>');
        for (final t in timelineList) {
          if (trackPosition >= t.externalAudioTracks.length) {
            continue;
          }
          final track = t.externalAudioTracks[trackPosition];
          if (!_hasExternalAudioTrack(track)) {
            continue;
          }
          final startFrame =
              videoPositions[t.videoFileId]! + _msToFrames(track.trimStartMs);
          final durFrames = _msToFrames(track.durationMs);
          final endFrame = startFrame + durFrames;
          final trackKey = _externalTrackKey(t.videoFileId, track.mediaFileId);
          final audioClipId = externalAudioClipIds[trackKey]!;
          final videoClipId = videoClipIds[t.videoFileId]!;
          final audioFileId = externalAudioFileIds[trackKey]!;
          final sourceInFrames = _msToFrames(track.effectiveSourceInMs);
          final sourceOutFrames = sourceInFrames + durFrames;
          final sourceDurationFrames = _msToFrames(
            track.effectiveFileDurationMs,
          );

          buf.writeln('          <clipitem id="$audioClipId">');
          buf.writeln('            <name>${_xmlEscape(track.fileName)}</name>');
          buf.writeln('            <duration>$durFrames</duration>');
          buf.writeln('            <rate>');
          buf.writeln('              <timebase>$_fps</timebase>');
          buf.writeln('              <ntsc>FALSE</ntsc>');
          ...
          buf.writeln('              </rate>');
          buf.writeln(
            '              <name>${_xmlEscape(track.fileName)}</name>',
          );
          buf.writeln(
            '              <pathurl>${_toFileUri(track.effectiveAudioPath)}</pathurl>',
          );
          buf.writeln('              <media>');
          buf.writeln('                <audio>');
          buf.writeln('                  <samplecharacteristics>');
  ...
  static bool _hasExternalAudioTrack(ExternalAudioTrackData track) {
    return track.effectiveAudioPath.isNotEmpty && track.durationMs > 0;
  }

  static int _maxExternalTrackCount(List<TimelineData> timelineList) {
    var maxTracks = 0;
    for (final timeline in timelineList) {
      if (timeline.externalAudioTracks.length > maxTracks) {
        maxTracks = timeline.externalAudioTracks.length;
      }
    }
    return maxTracks;
  }

  static String _externalTrackKey(String videoFileId, String mediaFileId) {
    return '$videoFileId|$mediaFileId';
  }
