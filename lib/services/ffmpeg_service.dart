...
class FfmpegService {
  ...
  static final String _ffmpegName = Platform.isWindows
      ? 'ffmpeg.exe'
      : 'ffmpeg';
  static final String _ffprobeName = Platform.isWindows
      ? 'ffprobe.exe'
      : 'ffprobe';
  static String _ffmpegExecutable = _ffmpegName;
  static String _ffprobeExecutable = _ffprobeName;

  /// 设置 FFmpeg 目录（由 app.dart 从 settings 注入）
  static void setFfmpegDir(String dir) {
    final trimmed = dir.trim();
    if (trimmed.isEmpty) {
      _ffmpegExecutable = _ffmpegName;
      _ffprobeExecutable = _ffprobeName;
      return;
    }
    ...
    for (final candidateDir in [trimmed, p.join(trimmed, 'bin')]) {
      ...
      final ffprobe = p.join(candidateDir, _ffprobeName);
      if (File(ffmpeg).existsSync() && File(ffprobe).existsSync()) {
        _ffmpegExecutable = ffmpeg;
        _ffprobeExecutable = ffprobe;
        return;
      }
    }

    _ffmpegExecutable = p.join(trimmed, _ffmpegName);
    _ffprobeExecutable = p.join(trimmed, _ffprobeName);
  }

  static String get _ffmpeg => _ffmpegExecutable;
  static String get _ffprobe => _ffprobeExecutable;
