...
class SubtitlePrepareService {
  ...
  static List<SubtitleClip> _buildGeneratedAggregateClips({
    ...
    final sortedClips = [...localAudioClips]
      ..sort((left, right) {
        ...
        final leftGlobal =
            (leftMedia?.layoutStartMs ?? 0) +
            (left.localStartMs ?? left.startMs);
        final rightGlobal =
            (rightMedia?.layoutStartMs ?? 0) +
            (right.localStartMs ?? right.startMs);
    ...
    for (var index = 0; index < sortedClips.length; index++) {
      ...
      final globalStart =
          media.layoutStartMs + (clip.localStartMs ?? clip.startMs);
  ...
  static MediaFile? _bestMediaByFileName(
    ...
    final subtitleName = _normalizeName(subtitlePath.fileNameWithoutExtension);
    MediaFile? best;
    double bestScore = 0.0;
    for (final media in mediaFiles) {
      final mediaName = _normalizeName(media.filePath.fileNameWithoutExtension);
