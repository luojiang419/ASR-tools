...
class SubtitleMatchService {
  ...
  static Future<List<MediaFile>> getUnmatchedAudios(String projectId) async {
    ...
    final normalizedAudios = AudioGroupingService.applyImportGrouping(
      allAudios,
    );
    final results = await DatabaseService.getSyncResults(projectId);
    final matchedAudioIds = results
        ...
        .toSet();
    final matchedAudioFiles = normalizedAudios
        .where((audio) => matchedAudioIds.contains(audio.id))
        .toList(growable: false);
    final matchedGroupKeys = matchedAudioFiles
        .map((audio) => audio.audioGroupKey)
        .whereType<String>()
        .toSet();
    final subtitleCountByMediaId =
        await DatabaseService.getPreparedSubtitleCountByMediaId(projectId);
    final representatives = AudioGroupingService.selectRepresentativeTracks(
      normalizedAudios,
      subtitleCountByMediaId: subtitleCountByMediaId,
    );
    return representatives
        .where((audio) {
          if (matchedAudioIds.contains(audio.id)) {
            return false;
          }
          final groupKey = audio.audioGroupKey;
          if (groupKey == null || groupKey.trim().isEmpty) {
            return true;
          }
          return !matchedGroupKeys.contains(groupKey);
        })
        .toList(growable: false);
  }
  ...
  static Future<_MatchRuntimeData> _buildMatchRuntimeData(
    ...
    final subtitleCountByMediaId =
        await DatabaseService.getPreparedSubtitleCountByMediaId(projectId);
    final representativeAudios =
        AudioGroupingService.selectRepresentativeTracks(
          audios,
          subtitleCountByMediaId: subtitleCountByMediaId,
        );
    final representativeAudioIds = representativeAudios
        .map((audio) => audio.id)
        .toSet();
    final audioWindows = await DatabaseService.getSubtitleWindows(
      projectId,
