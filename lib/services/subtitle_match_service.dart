...
class SubtitleMatchService {
  ...
  static Future<SyncResult> createManualMatch({
    ...
    await DatabaseService.replaceSyncAudioSegmentsForResult(result.id, [
      _buildSingleSegment(
  ...
  static Future<SyncResult> manualAnchorMatch({
    ...
    final updated = existing.copyWith(
      ...
      offsetMadMs: 0,
      alignmentCoverage: existing.videoDurationMs <= 0
          ? 0.0
    ...
    await DatabaseService.replaceSyncAudioSegmentsForResult(syncResultId, [
      _buildSingleSegment(
  ...
  static String _buildManualAnchorNotes({
    ...
    return '手动匹配 | 总字幕锚点 ${_formatClock(anchorTime)} | '
        '目标音频 ${targetAudio.filePath.fileName} | '
    ...
    for (var index = 0; index < videos.length; index++) {
      ...
      candidates.add(
        MatchCandidate(
          ...
          videoFileId: video.id,
          audioFileId:
              alignment.primaryAudioFileId ??
              (audios.isEmpty ? '' : audios.first.id),
  ...
  static _AggregateCandidate? _scoreAggregateCandidate({
    ...
    final contextScore =
        (cluster.entries.length / math.max(videoWindows.length, 1)).clamp(
          0.0,
          1.0,
        );
    final anchorScore = _anchorStabilityScore(cluster.offsets);
    final uniquenessScore =
        cluster.entries.fold<double>(
          0.0,
    ...
    final neighborScore = previousFinalOffsetMs == null
        ? 0.5
        : (1.0 -
                  ((cluster.weightedMedianOffsetMs - previousFinalOffsetMs)
                          .abs() /
                      5000))
              .clamp(0.0, 1.0);
  ...
  static _AggregateAlignmentResult _alignVideoToAggregateAudio({
    ...
    final finalOffsetMs = _medianInt(
    ...
    final dominantSegment = [...segments]
      ..sort(
        (left, right) => right.videoDurationMs.compareTo(left.videoDurationMs),
      );
    final primarySegment = dominantSegment.isEmpty
        ? null
        : dominantSegment.first;
    ...
    return _AggregateAlignmentResult(
      ...
      summaryAudioSourceInMs: segments.isEmpty
          ? null
          : segments.first.audioSourceInMs,
      summaryAudioSourceOutMs: segments.isEmpty
          ? null
          : segments.first.audioSourceOutMs,
      sourceClamped: sourceClamped,
      audioTooShort: audioTooShort,
      timelineOffsetMs: timelineOffsetMs,
      ...
      sourceClampedReason: clampReasons.isEmpty
          ? null
          : clampReasons.toSet().join(' | '),
      notes:
  ...
  static List<_MatchedAggregateAnchor> _selectMonotonicAggregateAnchors({
    ...
    for (final videoClip in videoClips) {
      ...
      for (
        ...
        final offsetScore = (1.0 - ((offsetMs - rollingOffset).abs() / 6000))
            .clamp(0.0, 1.0);
      ...
      final matchedAudio = _resolveAudioForGlobalMs(
        audios,
        best.audioClip.startMs,
      );
  ...
  static List<_AnchorGroup> _groupAnchorsBySegment(
    ...
    for (final anchor in anchors.skip(1)) {
      final medianOffset = _medianInt(
        current.map((item) => item.offsetMs).toList(),
      );
  ...
  static int _resolveSegmentBoundary(
    ...
    final rawBoundary = ((projectedFromCurrent + projectedFromNext) / 2)
        .round();
    return math
            .max(minBoundary, math.min(maxBoundary, rawBoundary))
            .clamp(0, videoDurationMs)
        as int;
  }
  ...
  static List<_SegmentDraft> _mergeSegmentDrafts(List<_SegmentDraft> drafts) {
    ...
    for (final current in sorted.skip(1)) {
      ...
      merged[merged.length - 1] = previous.copyWith(
        ...
        sourceClampedReason:
            previous.sourceClampedReason ?? current.sourceClampedReason,
        notes:
            '${previous.notes ?? ''}${previous.notes == null ? '' : ' | '}${current.notes ?? ''}',
      );
    }
  ...
  static _OffsetClusterSelection? _selectDominantOffsetCluster(
    ...
    int previousOffset =
        sorted.first.value.audioWindow.startMs -
        sorted.first.value.videoWindow.startMs;
  ...
  static double _clusterWeight(
    ...
    return cluster.fold<double>(
      ...
      (sum, entry) =>
          sum +
          (entry.value.textScore * entry.value.audioWindow.uniquenessWeight),
    );
  }
    ...
    final weighted =
        cluster
            .map(
              (entry) => (
                offset:
                    entry.value.audioWindow.startMs -
                    entry.value.videoWindow.startMs,
                weight:
                    entry.value.textScore *
                    entry.value.audioWindow.uniquenessWeight,
              ),
            )
            .toList()
...
class _WorkerMedia {
  ...
  factory _WorkerMedia.fromMap(Map<String, dynamic> map) {
    ...
    return _WorkerMedia(
      ...
      directory: filePath.directoryPath.toLowerCase(),
...
class _AnchorGroup {
  ...
  int get medianOffsetMs => SubtitleMatchService._medianInt(
    anchors.map((item) => item.offsetMs).toList(),
  );
...
class _SegmentDraft {
  ...
  _SegmentDraft copyWith({
    ...
    return _SegmentDraft(
      ...
      sourceClampedReason: sourceClampedReason ?? this.sourceClampedReason,
