...
class DatabaseService {
  ...
  static Future<void> _createMediaFilesTable(Database db) async {
    await db.execute('''
      CREATE TABLE media_files (
        ...
        thumbnail_path TEXT,
        audio_group_key TEXT,
        audio_group_label TEXT,
        audio_track_kind TEXT NOT NULL DEFAULT 'single',
        audio_track_index INTEGER,
        audio_is_primary INTEGER NOT NULL DEFAULT 0,
  ...
  static Future<void> _migrateToV5(Database db) async {
    await _safeAddColumn(db, 'media_files', 'audio_group_key TEXT');
    await _safeAddColumn(db, 'media_files', 'audio_group_label TEXT');
    await _safeAddColumn(
      db,
      'media_files',
      "audio_track_kind TEXT NOT NULL DEFAULT 'single'",
    );
    await _safeAddColumn(db, 'media_files', 'audio_track_index INTEGER');
    await _safeAddColumn(
      db,
      'media_files',
      'audio_is_primary INTEGER NOT NULL DEFAULT 0',
    );
    await _backfillAudioGroupingFields(db);
  }
  ...
  static Future<void> _ensureCurrentSchema(Database db) async {
    ...
    if (!mediaColumnNames.contains('audio_group_key')) {
      await _safeAddColumn(db, 'media_files', 'audio_group_key TEXT');
    }
    if (!mediaColumnNames.contains('audio_group_label')) {
      await _safeAddColumn(db, 'media_files', 'audio_group_label TEXT');
    }
    if (!mediaColumnNames.contains('audio_track_kind')) {
      await _safeAddColumn(
        db,
        'media_files',
        "audio_track_kind TEXT NOT NULL DEFAULT 'single'",
    ...
    if (!mediaColumnNames.contains('audio_track_index')) {
      await _safeAddColumn(db, 'media_files', 'audio_track_index INTEGER');
    }
    if (!mediaColumnNames.contains('audio_is_primary')) {
      await _safeAddColumn(
        db,
        'media_files',
        'audio_is_primary INTEGER NOT NULL DEFAULT 0',
  ...
  static Future<void> _backfillAudioGroupingFields(Database db) async {
    final maps = await db.query(
      'media_files',
      where: 'type = ?',
      whereArgs: [MediaType.audio.name],
      orderBy: 'sort_index ASC, file_path COLLATE NOCASE ASC',
    );
    if (maps.isEmpty) return;

    final files = maps.map((row) => MediaFile.fromMap(row)).toList();
    final grouped = AudioGroupingService.applyImportGrouping(files);
    final batch = db.batch();
    for (final file in grouped) {
      batch.update(
        'media_files',
        {
          'audio_group_key': file.audioGroupKey,
          'audio_group_label': file.audioGroupLabel,
          'audio_track_kind': file.audioTrackKind.name,
          'audio_track_index': file.audioTrackIndex,
          'audio_is_primary': file.audioIsPrimary ? 1 : 0,
        },
        where: 'id = ?',
        whereArgs: [file.id],
      );
  ...
  static Future<SyncReviewDetail?> getSyncReviewDetail(
    ...
    final allAudioFiles = await getMediaFiles(
      syncResult.projectId,
      type: MediaType.audio,
    );
    final subtitleCountByMediaId = await getPreparedSubtitleCountByMediaId(
      syncResult.projectId,
    );
    final audioCandidates = AudioGroupingService.selectRepresentativeTracks(
      allAudioFiles,
      subtitleCountByMediaId: subtitleCountByMediaId,
    );
    final audioGroupTracks = audioFile == null
        ? const <MediaFile>[]
    ...
    return SyncReviewDetail(
      ...
      audioGroupTracks: audioGroupTracks,
