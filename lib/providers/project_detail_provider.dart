...
const _projectFieldUnchanged = Object();
...
class ProjectDetailNotifier extends AsyncNotifier<ProjectDetailState> {
  ...
  Future<void> clearMediaFiles(MediaType type) async {
    ...
    final files = type == MediaType.video
        ? current.videoFiles
        : current.audioFiles;
    if (files.isEmpty) return;

    if (type == MediaType.video) {
      await VideoThumbnailService.deleteProjectCache(projectId);
    }
    await DatabaseService.deleteMediaFiles(projectId, type: type);
    await _updateProjectDirectories(
      videoDirectory: type == MediaType.video ? null : _projectFieldUnchanged,
      audioDirectory: type == MediaType.audio ? null : _projectFieldUnchanged,
    );
    await _invalidatePreparedWorkflow(projectId);
    await loadProject(projectId);
  }
  ...
  Future<void> reorderMedia(MediaType type, List<String> orderedIds) async {
    ...
    if (type == MediaType.audio) {
      await _refreshAudioGrouping(s.project!.id);
    }
    await _invalidatePreparedWorkflow(s.project!.id);
    await loadProject(s.project!.id);
  }
  ...
  Future<List<MediaFile>> _buildMediaFiles(
    ...
    for (final filePath in paths..sort()) {
      MediaProbeInfo? info;
      if (FfmpegService.canProbeMedia) {
        try {
  ...
  Future<void> _updateProjectDirectories({
    Object? videoDirectory = _projectFieldUnchanged,
    Object? audioDirectory = _projectFieldUnchanged,
  }) async {
    final current = state.valueOrNull;
    final project = current?.project;
    if (current == null || project == null) return;
    ...
    await DatabaseService.updateProject(updated);
    state = AsyncData(current.copyWith(project: updated));
  }

  Future<void> _refreshAudioGrouping(String projectId) async {
    ...
    final grouped = AudioGroupingService.applyImportGrouping(audioFiles);
    await DatabaseService.updateMediaFiles(grouped);
  }
