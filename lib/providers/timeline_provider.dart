  ...
  Future<void> batchTrim(String outputDir) async {
    ...
    try {
      ...
      final trimmedTimeline = List.generate(current.timelineList.length, (
        ...
        final result = index < outputs.length
            ? outputs[index]
            : const TimelineTrimResult();
        final timeline = current.timelineList[index];
        final tracks = timeline.externalAudioTracks
            .map(
              (track) => track.copyWith(
                trimmedAudioPath:
                    result.trackOutputByMediaId[track.mediaFileId],
              ),
            )
            .toList(growable: false);
        if (result.primaryOutputPath.isEmpty &&
            tracks.every((track) => !track.hasTrimmedAudio)) {
          return timeline;
        }
        return timeline.copyWith(
          trimmedAudioPath: result.primaryOutputPath,
          externalAudioTracks: tracks,
        );
      });
