...
class _SyncReviewPageState extends ConsumerState<SyncReviewPage> {
            ...
            children: [
              Expanded(
                child: _buildSubtitlePanel(
                  title: '视频字幕',
                  subtitleName: detail.videoFile.filePath.fileName,
              ...
              Expanded(
                child: _buildSubtitlePanel(
                  ...
                  subtitleName: detail.aggregateAudioSubtitleFile == null
                      ? '未检测到音频总字幕文件'
                      : detail.aggregateAudioSubtitleFile!.filePath.fileName,
  ...
  Widget _buildHeader(
    ...
    final videoName = detail.videoFile.filePath.fileName;
    final audioName = detail.audioFile == null
        ? '未匹配音频'
        : detail.audioFile!.filePath.fileName;
    final audioGroupSummary = detail.audioFile == null
        ? '未匹配音频组'
        : AudioGroupingService.buildGroupSummary(
            detail.audioFile!,
            detail.audioGroupTracks,
          );
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                '音频组: $audioGroupSummary',
                style: TextStyle(color: AppTheme.textSecondary, fontSize: 12),
                overflow: TextOverflow.ellipsis,
              ),
              if (detail.audioGroupTracks.length > 1) ...[
                const SizedBox(height: 4),
                Text(
                  '连带导出: ${detail.audioGroupTracks.map((file) => file.filePath.fileName).join(' / ')}',
                  style: TextStyle(color: AppTheme.textSecondary, fontSize: 12),
                  overflow: TextOverflow.ellipsis,
                ),
              ],
                ...
                children: [
                  ...
                  _resultLine(
                    ...
                    _manualPreview!.targetAudioFile == null
                        ? '--'
                        : _manualPreview!.targetAudioFile!.filePath.fileName,
                  ),
                  _resultLine(
                    'Source In',
