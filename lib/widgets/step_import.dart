...
import '../services/audio_grouping_service.dart';
...
class _StepImportState extends ConsumerState<StepImport> {
  ...
  Widget build(BuildContext context) {
    ...
    final audioGroupCount = state == null
        ? 0
        : AudioGroupingService.groupAudioFiles(state.audioFiles).length;
        ...
        children: [
          if (state != null && !state.isScanning) ...[
            _PrepareStatusPanel(
              ...
              audioGroupCount: audioGroupCount,
  ...
  Future<void> _pickDirectory(MediaType type) async {
    final result = await _runPickerAction(
      actionLabel: type == MediaType.video ? '选择视频目录' : '选择音频目录',
      action: () => FilePicker.platform.getDirectoryPath(
  ...
  Future<void> _pickMediaFiles(MediaType type) async {
    final result = await _runPickerAction(
      actionLabel: type == MediaType.video ? '选择视频文件' : '选择音频文件',
      action: () => FilePicker.platform.pickFiles(
        allowMultiple: true,
  ...
  Future<void> _pickSubtitleFiles(MediaType type) async {
    final result = await _runPickerAction(
      actionLabel: type == MediaType.video ? '选择视频字幕' : '选择音频字幕',
      action: () => FilePicker.platform.pickFiles(
        allowMultiple: true,
        type: FileType.custom,
  ...
  Future<void> _pickManifest(MediaType type) async {
    final result = await _runPickerAction(
      actionLabel: type == MediaType.video
          ? '选择视频顺序 manifest'
          : '选择音频顺序 manifest',
      action: () => FilePicker.platform.pickFiles(
        type: FileType.custom,
  ...
  Future<void> _confirmClearMedia(MediaType type, int fileCount) async {
    if (fileCount <= 0) return;
    final label = type == MediaType.video ? '视频素材' : '音频素材';
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) {
        return AlertDialog(
          title: Text('清空$label'),
          content: Text('确认清空当前已导入的$label吗？共 $fileCount 个文件。'),
          actions: [
            TextButton(
    ...
    await ref.read(projectDetailProvider.notifier).clearMediaFiles(type);
    if (mounted) {
      SnackbarUtil.success(context, '$label已清空');
    }
  }

  Future<T?> _runPickerAction<T>({
    ...
    try {
      ...
      if (mounted) {
        SnackbarUtil.error(context, '$actionLabel失败：$error');
      }
      return null;
    }
  }
              ...
              _SectionHeader(
                ...
                trailing: Wrap(
                  ...
                  children: [
                    ...
                    OutlinedButton.icon(
                      onPressed: widget.files.isEmpty ? null : widget.onClear,
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (file.type == MediaType.audio) ...[
                                        const SizedBox(height: 4),
                                        Text(
                                          '音频组 ${file.audioGroupLabel ?? file.filePath.fileName.split('.').first}  |  轨道 ${AudioGroupingService.trackDisplayLabel(file)}${file.audioIsPrimary ? ' | 主轨' : ''}',
                                          style: TextStyle(
                                            color: AppTheme.textSecondary,
                                            fontSize: 11,
                                          ),
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _StatusChip(label: '音频素材 $audioCount 文件 / $audioGroupCount 组'),
