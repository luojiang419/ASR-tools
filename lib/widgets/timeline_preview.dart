      ...
      child: Column(
        ...
        children: [
          ...
          if (timeline.externalAudioTracks.isEmpty) ...[
            ...
            _TrackBar(title: 'A1', color: AppTheme.highlight, label: '未匹配音频'),
          ] else
            ...List.generate(timeline.externalAudioTracks.length, (trackIndex) {
              final track = timeline.externalAudioTracks[trackIndex];
              return Padding(
                padding: EdgeInsets.only(top: trackIndex == 0 ? 6 : 4),
                child: _TrackBar(
                  title: 'A${trackIndex + 1}',
                  color: track.isPrimary
                      ? AppTheme.highlight
                      : AppTheme.success,
                  label:
                      '${track.fileName} | ${_formatMs(track.trimStartMs)} - ${_formatMs(track.trimEndMs)}${track.wasClamped ? ' | 已截断' : ''}',
                ),
              );
            }),
