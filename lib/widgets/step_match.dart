...
class _StepMatchState extends ConsumerState<StepMatch> {
  ...
  Widget _buildContent(MatchState state, ProjectDetailState? projectState) {
    final allAudioFiles = projectState?.audioFiles ?? const <MediaFile>[];
    final audioGroups = AudioGroupingService.groupAudioFiles(allAudioFiles);
    final subtitleCountByMediaId =
        projectState?.preparedSubtitleCountByMediaId ?? const <String, int>{};
    final audioGroupOptions = audioGroups
        .map(
          (group) => _AudioGroupOption(
            representativeFile: AudioGroupingService.selectRepresentativeTrack(
              group.files,
                        ...
                        child: visibleResults.isEmpty && !state.isMatching
                            ...
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final audioDisplayLabel = audio == null
                                      ? null
                                      : AudioGroupingService.buildGroupSummary(
                                          audio,
                                          AudioGroupingService.siblingTracksFor(
                                            allAudioFiles,
                                            audio,
                                          ),
                                        );
                                  ...
                                  return MatchResultTile(
                                    ...
                                    videoFile: video,
                                    audioFile: audio,
                                    audioDisplayLabel: audioDisplayLabel,
  ...
  Future<void> _showManualMatchDialog(
    ...
    List<_AudioGroupOption> audioGroups,
  ) async {
    String? selectedVideoId = videos.first.id;
    String? selectedAudioId = audioGroups.first.representativeFile.id;
                  ...
                  items: audioGroups
                      .map(
                        (group) => DropdownMenuItem(
                          value: group.representativeFile.id,
                          child: Text(
                            AudioGroupingService.buildGroupSummary(
                              group.representativeFile,
                              group.files,
                            ),
                            overflow: TextOverflow.ellipsis,
                          ),
...
class _AudioGroupOption {
  final MediaFile representativeFile;
  final List<MediaFile> files;

  const _AudioGroupOption({
